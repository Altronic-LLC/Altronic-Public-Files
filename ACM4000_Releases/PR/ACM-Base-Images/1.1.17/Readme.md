# [ACM Base Images 1.1.17](https://github.com/Altronic-LLC/Altronic-Public-Files/blob/main/ACM4000_Releases/PR/ACM-Base-Images/1.1.17/MDI_1.1.17.zip)

> **Copyright (c) 2025. All rights reserved.**
> Altronic, LLC
> 712 Trumbull Avenue
> Girard, Ohio 44420
> United States of America

## Release Kind: Preview Release

For demonstration and shakedown. May contain incomplete features,
bugs of various severity.

## Intended Use and Users

Early release of software intended for internal testing and evaluation. These versions are not final and may contain bugs or incomplete features. They are distributed for testing purposes and are not to be used for production environments or field deployment.

#### Versioning:
- **ACM Base Images:** 1.1.17

### Changelog:

#### New Items:
- **ACM Base Image 1.1.17:** Updated MDI Base Image For Field Updates, Starts at 1.1.17 MDI Version.

## Where To Find This Release

### ACM Base Images version 1.1.17

[ACM Base Images 1.1.17](https://github.com/Altronic-LLC/Altronic-Public-Files/blob/main/ACM4000_Releases/PR/ACM-Base-Images/1.1.17/MDI_1.1.17.zip)

## Update Instructions

1. **Download ACM Base Images 1.1.17**: [Download from GitHub](https://github.com/Altronic-LLC/Altronic-Public-Files/raw/refs/heads/main/ACM4000_Releases/PR/ACM-Base-Images/1.1.17/MDI_1.1.17.zip?download=)
2. Follow the [ACM Imaging Instructions](https://www.altronic-llc.com/wiki-doc/acm-4000/acm-4000-imaging-guide/) to install the base image

## Target, Tools, and Equipment

### Required Hardware
- ACM-4000 device
- USB Flash Drive (≥ 2GB)

### For Base Image Installation
- **Software Tools**:
  - RPIboot utility
  - Raspberry Pi Imager application
- **Additional Hardware**:
  - USB-A to USB-C Cable from PC(USB A) to ACM-4000(USB-C) connected to USB OTG Port
  - Jumper wire for RPI boot pin on the ACM 3rd PIN Over from DO1

### Imaging Process Notes
Complete imaging instructions can be found in the [ACM Imaging Instructions](https://www.altronic-llc.com/wiki-doc/acm-4000/acm-4000-imaging-guide/) documentation.
