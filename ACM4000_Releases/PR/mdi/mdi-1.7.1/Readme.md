# [MDI 1.7.1](https://github.com/Altronic-LLC/Altronic-Public-Files/blob/main/ACM4000_Releases/PR/mdi/mdi-1.7.1/mdi-1.7.1.atf)
...
This release supersedes [MDI 1.7.0](https://github.com/Altronic-LLC/Altronic-Public-Files/blob/main/ACM4000_Releases/PR/mdi/mdi-1.7.0/) and [MDI 1.6.3](https://github.com/Altronic-LLC/Altronic-Public-Files/blob/main/ACM4000_Releases/PR/mdi/mdi-1.6.3/) (1.6.3 was withdrawn as a Release Candidate).
#### Versioning:
- **MDI:** 1.7.1
- **Master AWI:** 2.0.7
...
##### Bug Fixes:
- **The Screen Stays Responsive When Changing The Layout:** On legacy ACM-4000 units, changing the layout or saving settings while the DE-4000 was not communicating could leave the screen unresponsive. Saving no longer reloads the screen, the device buttons no longer wait on the DE-4000 to appear, and the screen keeps responding while the DE-4000 is disconnected.
- **Switching Between Devices Is Immediate:** Pressing a device button on the HMI now switches in well under a second, without the blank panel and connection prompt that used to show in between.
- **Better Behaviour When Memory Runs Low:** Legacy ACM-4000 units have more free memory, and when memory does run low the unit slows down gracefully instead of stalling.
- **Clock Corrections:** The unit's clock is now corrected from the DE-4000 only when it has actually drifted, instead of being nudged every few seconds.

##### Quality of Life(QOL):
- **Master AWI 2.0.7:** The default Master AWI is now version 2.0.7. Units still on 2.0.6 are moved to 2.0.7 by this update; a Master AWI you uploaded yourself is kept if it is the same version or newer.
- **Diagnostics For Support:** The unit keeps a short log of the screen software while it runs, so support can see what happened if a problem is reported.
...
- **Your AWI Version Is Kept When Importing:** Importing a configuration no longer overwrites the Master AWI already on the unit. The default Master AWI shipped with the unit is now version 2.0.7.
...
### ACM-4000 bootloader update version 1.7.1
...
1. **Download MDI 1.7.1**: [Download from GitHub](https://github.com/Altronic-LLC/Altronic-Public-Files/raw/refs/heads/main/ACM4000_Releases/PR/mdi/mdi-1.7.1/mdi-1.7.1.atf?download=)
